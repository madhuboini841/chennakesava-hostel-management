...
def send_fast2sms(mobile_number, message, student_id=None, fee_id=None):
    ...
    try:
        ...
        if res_json.get("return"):
            ...
            status = "failed"
            error_msg = res_json.get("message", "Unknown API Error")
    except Exception as e:
        status = "error"
        error_msg = str(e)
    ...
    if conn:
        ...
        if student_id:
            ...
            try:
                cursor.execute("INSERT INTO sms_logs (mobile_number, message, status, error_message) VALUES (%s, %s, %s, %s)",
                              (mobile_number, message, status, error_msg))
            except Exception as e:
                print(f"[SMS LOG ERROR] {e}")
                conn.rollback()
                cursor.execute("INSERT INTO sms_logs (mobile_number, message, status) VALUES (%s, %s, %s)",
                              (mobile_number, message, status))
...
def admin_dashboard():
    ...
    cursor.execute("SELECT COUNT(*) as c FROM sms_logs WHERE DATE(sent_at) = CURRENT_DATE")
    ...
    try:
        cursor.execute("SELECT * FROM students WHERE status = 'pending' ORDER BY created_at DESC")
...
def send_manual_sms():
    ...
    if student:
        ...
        success, err = send_fast2sms(numbers, message, student_id=student_id)
        if success:
            flash("SMS sent successfully!", "success")
        else:
            flash(f"Failed to send SMS: {err}", "error")
